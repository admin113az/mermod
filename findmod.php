...
$sql = "SELECT * FROM skins ORDER BY created_at DESC";
...
<head>
    ...
    <title>List skins Lẻ - HD MOD</title>
    <script async
    ...
    <div class="container">
            ...
            <div class="luuy">
                <h2><i class='bx bxs-right-arrow-alt bx-tada'></i> Những bản mod
                    lẻ này được upload sau khi liên quân UPDATE NGÀY
                    <b class="new"><?php echo htmlspecialchars($latestUpdateDate); ?></b> </h2>
                <br><h3><i class='bx bx-chevron-right bx-tada'></i>Tất cả các
                    file mod lẻ đều phải dán theo cách "dán từng file" hoặc zip resource lại trỏ bằng menu !!! Ai
                ...
                <br><h3><i class='bx bxs-right-arrow-alt bx-tada'></i> Ad sẽ
                    update skin lẻ mỗi ngày, vậy nên thiếu skin nào thì AE có
                    thể vào link này check mỗi ngày <i
        ...
        <?php if ($result->num_rows > 0): ?>
            <?php while($row = $result->fetch_assoc()): ?>
                <a href="<?php echo $row['link']; ?>" target="_blank" class="st-heroes__item">
                    <div class="st-heroes__item--img">
                        <img src="<?php echo $row['image_url']; ?>" alt="<?php echo $row['alt_text']; ?>">
                    </div>
            ...
            <?php endwhile; ?>
        <?php else: ?>
            <p>Chưa có skin lẻ nào được upload lên web !! vui lòng đợi admin upload</p>
        <?php endif; ?>
    </div>
